...
//
// Bootup the masternode, look for a 250000 LEGN input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Legion.conf */
...
{
    CLegionAddress address(collateralAddress);
