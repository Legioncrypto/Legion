...
namespace {
    class CLegionAddressVisitor : public boost::static_visitor<bool> {
    private:
        CLegionAddress *addr;
    public:
        CLegionAddressVisitor(CLegionAddress *addrIn) : addr(addrIn) { }
...
bool CLegionAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CLegionAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CLegionAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CLegionAddressVisitor(this), dest);
}

bool CLegionAddress::IsValid() const {
...
CTxDestination CLegionAddress::Get() const {
    if (!IsValid())
...
bool CLegionAddress::GetKeyID(CKeyID &keyID) const {
...
bool CLegionAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CLegionSecret::SetKey(const CKey& vchSecret) {
...
CKey CLegionSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CLegionSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CLegionSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CLegionSecret::SetString(const std::string& strSecret) {
