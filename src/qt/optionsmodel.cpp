...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLegionAmount"))
        settings.setValue("nAnonymizeLegionAmount", 1000);
    nAnonymizeLegionAmount = settings.value("nAnonymizeLegionAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeLegionAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeLegionAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeLegionAmount:
            return QVariant(nAnonymizeLegionAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeLegionAmount:
            nAnonymizeLegionAmount = value.toInt();
            settings.setValue("nAnonymizeLegionAmount", nAnonymizeLegionAmount);
            emit AnonymizeLegionAmountChanged(nAnonymizeLegionAmount);
            break;
        default:
            break;
        }
