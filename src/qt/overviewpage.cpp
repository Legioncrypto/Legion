...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLegionAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLegionAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeLegionAmount = strAnonymizeLegionAmount.remove(strAnonymizeLegionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLegionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeLegionAmount*COIN) nMaxToAnonymize = nAnonymizeLegionAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeLegionAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeLegionAmount));
        strAnonymizeLegionAmount = strAnonymizeLegionAmount.remove(strAnonymizeLegionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLegionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
