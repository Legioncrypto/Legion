...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CLegionSecret::SetString tests isValid, whereas CLegionAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CLegionAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CLegionAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CLegionAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CLegionAddressVisitor(&dummyAddr), nodest));
